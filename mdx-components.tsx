...
export function useMDXComponents(components: MDXComponents): MDXComponents {
	return {
		...
		h2: ({ children }) => (
			<h2 className="text-3xl font-bold mt-6 mb-3 text-black dark:text-white border-b border-gray-200 dark:border-gray-700 pb-1">
				{children}
			</h2>
		),
		h3: ({ children }) => (
		...
		code: ({ children }) => {
			...
			const isInline =
				typeof children === 'string' && !(children as any)?.props?.className?.includes('language-');
		...
		Callout: ({ children, type = 'default' }) => (
			<Callout type={type} className="my-4">
				{children}
			</Callout>
		),
		...
		Tabs: ({ children, defaultValue }) => (
			<TabsUI defaultValue={defaultValue} className="my-4">
				{children}
			</TabsUI>
		),
