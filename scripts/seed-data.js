...
async function seedDatabase() {
	...
	const { error: speciesTableError } = await supabase.rpc('create_species_table_if_not_exists');
	if (speciesTableError) {
		console.error('Error creating species table:', speciesTableError);

		// Try alternative approach with SQL query
		const { error: createSpeciesError } = await supabase.query(`
    ...
    `);
		...
		if (createSpeciesError) {
			console.error('Error with SQL approach for species table:', createSpeciesError);
		}
	}

	// Create blog_posts table
	const { error: blogTableError } = await supabase.rpc('create_blog_posts_table_if_not_exists');
	if (blogTableError) {
		console.error('Error creating blog_posts table:', blogTableError);

		// Try alternative approach with SQL query
		const { error: createBlogError } = await supabase.query(`
    ...
    `);
		...
		if (createBlogError) {
			console.error('Error with SQL approach for blog_posts table:', createBlogError);
		}
	}
